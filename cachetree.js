...
export default class CacheTree {
  ...
  _search(path, cache, filter) {
    const [currNode, ...remainingPath] = path;
    ...
    } else if (filter.hasOwnProperty(currNode) && isArray(filter[currNode])) {
      forEach(filter[currNode], (item) => {
        if (!cache.hasOwnProperty(item)) {
          console.log(`missing parameter: ${currNode}: ${item}`);
        }
      });

      const trimmedCache = pick(cache, filter[currNode]);

      return flatMap(trimmedCache, (subCache) => this._search(remainingPath, subCache, filter));
    } else if (filter.hasOwnProperty(currNode)) {
      if (!cache.hasOwnProperty(filter[currNode])) {
        return [];
      }

      return this._search(remainingPath, cache[filter[currNode]], filter);
    }

    // select all at this level
    return flatMap(cache, (subCache) => this._search(remainingPath, subCache, filter));
  }
